...
def inception_v3_base(inputs, scope=None):
    ...
    with tf.variable_scope(scope, 'InceptionV3', [inputs]):
        ...
        with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d], stride=1, padding='VALID'):
            # slim.conv2d参数分别为(输入tensor，输出通道数，卷积核的尺寸， 步长， padding)
        ...
        with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d], stride=1, padding='SAME'):
            with tf.variable_scope('Mixed_5b'):
                ...
                # 将4个分支按照axis=3的维度合并，也就是按照输出的通道合并
            ...
            # 将Mixed_6e存储与end_points中，作为Auxiliary Classifier辅助模型的分类
            end_points['Mixed_6e'] = net

            with tf.variable_scope('Minxed_7a'):
                with tf.variable_scope('Branch_0'):
                    branch_0 = slim.conv2d(net, 192, [1, 1], scope='Conv2_0a_1x1')
        ...
        with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
            ...
            with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d], stride=1, padding='SAME'):
                ...
                with tf.variable_scope('AuxLogits'):
                    ...
                    aux_logits = slim.conv2d(aux_logits, 784, [5, 5], weights_initializer=trunc_normal(0.01),
                                             padding='VALID', scope='Conv2d_2a_5x5')
                    aux_logits = slim.conv2d(aux_logits, num_classes, [1, 1], normalizer_fn=None, activation_fn=None,
                                             weights_initializer=trunc_normal(0.01), scope='Conv2d_2b_1x1')
                    if spatial_squeeze:
